...
function calcOperation(operands){
    let rpnList = []
    let stack = []
    for (let token of operands){
        ...
        else if (OPERANDS[token] !== 'undefined'){
            while ( (stack.length > 0) &&
                    (OPERANDS[stack[stack.length-1]] >= OPERANDS[token])){
...
function expressionCalculator(expr) {
    ...
    let rpn = calcOperation(operands);
    return evalOperation(rpn)
}
